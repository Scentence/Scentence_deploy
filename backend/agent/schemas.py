...
class ChatRequest(BaseModel):
    ...
    user_mode: Optional[str] = Field(
        "BEGINNER", description="사용자 모드 (BEGINNER 또는 EXPERT)"
    )
...
class AgentState(Dict):
    ...
    constraint_source: Literal["explicit_user", "inferred", "system_default"] | None = (
        None
    )

    # [★Wave 2] 상태 분류 필드 (OK/NO_RESULTS/ERROR 통합 관리)
    chat_outcome_status: Optional[Literal["OK", "NO_RESULTS", "ERROR"]]
    chat_outcome_reason_code: Optional[
        str
    ]  # 예: "partial_results", "tool_error", "no_candidates"
...
class UserPreferences(BaseModel):
    ...
    reference_brand: Optional[str] = Field(
        None, description="참고 브랜드 (유사한 향 찾기 - 소프트 필터)"
    )
    brand: Optional[str] = Field(
        None, description="특정 브랜드 (해당 브랜드만 - 하드 필터)"
    )
    ...
    accord: Optional[str] = Field(
        None,
        description="선호하는 향의 분위기나 계열 (예: Woody, Floral, Citrus, Spicy)",
    )
    note: Optional[str] = Field(
        None,
        description="구체적으로 선호하는 향 원료 (예: Rose, Vetiver, Sandalwood, Vanilla)",
    )

    # [★추가] 추천 개수 (사용자가 명시한 경우)
    recommended_count: Optional[int] = Field(
        None, description="사용자가 요청한 추천 향수 개수"
    )

    # [★추가] 제외 브랜드 (말고/제외/빼고)
    exclude_brands: Optional[List[str]] = Field(
        None, description="검색에서 제외할 브랜드 목록 (최대 5개, 정규화된 브랜드명)"
    )

    # [★추가] 사용 목적 구분 (본인용 vs 선물용)
    use_case: Optional[Literal["SELF", "GIFT"]] = Field(
        None, description="사용 목적 (SELF: 본인용, GIFT: 선물용)"
    )
...
class ValidationResult(BaseModel):
    ...
    is_unsupported: bool = Field(description="True면 지원 불가능, False면 지원 가능")
    unsupported_category: Optional[str] = Field(
        None,
        description="불가능한 경우 카테고리: '제형', '성능', '가격', '레이어링', '구매정보', '인사' 등",
    )
    reason: str = Field(description="판단 이유 설명")
...
class InfoRoutingDecision(BaseModel):
    ...
    target_brand: Optional[str] = Field(
        None, description="[Phase 2] 브랜드 이름 (예: 'Dior', 'Jo Malone', 'Chanel')"
    )
...
class AdvancedSearchInput(BaseModel):
    ...
    exclude_brands: Optional[List[str]] = Field(
        default=None,
        description="검색에서 제외할 브랜드 목록 (정규화된 브랜드명, 최대 5개)",
    )
...
class PerfumeIdSearchInput(BaseModel):
    ...
    perfume_id: int = Field(description="조회할 향수의 perfume_id (예: 12345)")
